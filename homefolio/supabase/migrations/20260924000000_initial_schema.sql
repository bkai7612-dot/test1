...
-- Files are stored in the private "homefolio" bucket; these rows hold metadata.
...
create policy "Users read own files" on storage.objects
  ...
  using (bucket_id = 'homefolio' and (storage.foldername(name))[1] = (select auth.uid())::text);

create policy "Users upload own files" on storage.objects
  for insert to authenticated
  with check (bucket_id = 'homefolio' and (storage.foldername(name))[1] = (select auth.uid())::text);
