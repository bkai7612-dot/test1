...
export function Logo({ iconOnly }: { iconOnly?: boolean }) {
  return (
    <Link to="/" className="inline-flex shrink-0 items-center gap-2.5 rounded-lg" aria-label="HomeHub home">
      <LogoMark />
      {!iconOnly && <span className="text-ink text-lg font-semibold tracking-tight">HomeHub</span>}
    </Link>
  );
}
