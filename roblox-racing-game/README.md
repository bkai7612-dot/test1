...
| **Race types** | Vote on **Circuit** (laps), **Sprint** (a long point-to-point road, about 6,000 studs, in the map's theme; first across the finish line wins) or **Elimination** (last place is knocked out every lap until one is left; needs 3+ racers) along with the map. |
| **Team races** | A Team Arena in the lobby has five pads (1v1, 2v2, 3v3, 4v4, 5v5); walk onto one or use the TEAM RACES menu. When a queue fills, a Red vs Blue match starts on its own track. Teams are balanced by level, points go by place (25, 18, 15, 12, 10...), and every member of the winning team gets +250 coins and +100 XP. You can free drive while you wait. |
| **Free drive** | Cruise any of the 8 circuits with no timer. Lap times pop up each time you cross the line. Use the FREE DRIVE menu or the kiosk in the lobby; press F or EXIT to leave. |
| **Cosmetic items** | Bought once with coins, then equipped on any car from CUSTOMIZE (first click previews, second click buys). **Wings:** Angel, Bat, Butterfly, Dragon, Jet. **Eyes:** Cute, Angry, Lashes, Love, Robot visor (armoured housing with a sweeping scanner light and antennas). **Trails:** Blue Streak, Fire, Ice, Toxic, Gold, Rainbow. Wings, eyes and a trail are separate slots, so a car can wear one of each at once. Trails are a flat ribbon just above the road, so they never block the car behind. |
| **Tracks** | 7 lanes wide with dashed lane lines and a 4-wide starting grid. The night tracks (Downtown Circuit, Neon Drift, Volcano Ring) have street lights on both sides that light up the road. |
...
4. After publishing, set the server size to **20** (Creator Dashboard → your game → Places → Server size) so 5v5 team races (10 players) can fill while others use the lobby.
